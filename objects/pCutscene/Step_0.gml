...
{
	spd = 0;
	if (keyboard_check(vk_enter)) && (animation_ended)
	{
...
// create cursor blinker at the end of text to prompt user to continue
if (spd == 0) && (animation_ended)
{
	cursorblink = Wave(0,60,1,0);
	if (cursorblink > 30)
	{
		string_delete(str, string_length(print) + 1, 1);
	}
	else
	{
		print = string_insert(" << press enter", str, string_length(print) + 1);
	}
}
...
/* the following code is needed in order to satify requirement to continue though cutscene
switch (next)
{
	default: animation_ened = true; show_debug_message("animation ended") break;
}*/
#endregion 